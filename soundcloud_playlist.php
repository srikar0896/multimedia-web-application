...
<p style="font-style:bold">VISIT</p>
<center>
...
$query="SELECT `sc_name`,`sc_link` FROM `soundcloud_playlist` ORDER By `sc_id` DESC";

$query_run=mysql_query($query);

while($result_array=mysql_fetch_array($query_run,MYSQL_ASSOC)){
   $channel = array(
      'description' =>$result_array['sc_name']);
   ...
   $url=$result_array['sc_link'];
   $link=$url.'&amp;auto_play=false&amp;hide_related=false&amp;show_comments=false&amp;show_user=false&amp;show_reposts=false&amp;visual=false';
 echo '<center>';
 
  echo "<iframe width='560' height='195' scrolling='no' frameborder='no' src='$link'>";
  echo '</iframe>';
